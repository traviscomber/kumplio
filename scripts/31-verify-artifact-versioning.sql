...
begin
  ...
  if function_definition not like '%pg_advisory_xact_lock%'
    ...
    or function_definition not like '%parent_artifact_id%'
    or function_definition not like '%agent_runs%'
    or function_definition not like '%workflowId%'
    or function_definition not like '%stageIndex%' then
    raise exception 'Artifact version preparation is missing inferred lineage, concurrency or hashing';
  end if;
  ...
  if function_definition not like '%Artifact version content and lineage are immutable%'
    ...
    or function_definition not like '%Superseded artifact versions are immutable%'
    or function_definition not like '%same lineage%' then
