...
assert.match(workflow, /Create or resume isolated E2E workflow/)
assert.match(workflow, /workflowType:'compliance_assessment'/)
assert.match(workflow, /\$BASE_URL\/api\/agents\/workflows/)
assert.match(workflow, /echo "WORKFLOW_ID=\$WORKFLOW_ID" >> "\$GITHUB_ENV"/)
assert.match(workflow, /echo "TOTAL_STAGES=\$TOTAL_STAGES" >> "\$GITHUB_ENV"/)
assert.match(workflow, /console\.log\(w\.id\+' '\+n\)/, 'Bash read input must end with a newline so set -e does not abort at EOF')
assert.match(workflow, /w\.status==='completed'/)
assert.doesNotMatch(workflow, /WORKFLOW_ID:\s*[0-9a-f]{8}-[0-9a-f-]{27,}/i)
assert.doesNotMatch(workflow, /for stage_index in 0 1 2 3 4/)
assert.match(workflow, /for \(\(stage_index=0; stage_index<TOTAL_STAGES; stage_index\+\+\)\); do/)
assert.match(workflow, /Number\(process\.env\.TOTAL_STAGES\)-1/)
assert.match(workflow, /Number\(p\.workflow\?\.total_stages\) !== expectedStages/)
