...
assert.match(entry, /Ver orientación inicial/)
assert.match(entry, /Orientación inicial · sin registro/)
assert.match(entry, /Obtener diagnóstico completo/)
assert.match(entry, /What do you need to protect or resolve\?/)
assert.match(entry, /See initial guidance/)
assert.match(entry, /Get the complete assessment/)
...
assert.match(signUp, /Tu orientación está guardada/)
assert.match(signUp, /parseGuidedOnboardingDraft/)
assert.match(signUp, /localStorage\.getItem/)
assert.match(signUp, /no tendrás que escribir nuevamente tu situación/)

assert.match(layout, /PUBLIC_SITE_METADATA/)
assert.match(metadataCopy, /Sistema operativo de cumplimiento para Chile/)
assert.match(metadataCopy, /Sistema operativo de cumplimiento, documentos, requisitos y evidencia para organizaciones y personas en Chile/)
assert.match(metadataCopy, /Compliance operating system for Chile/)
assert.match(metadataCopy, /Compliance operating system for people, documents, requirements, expirations and evidence in Chile/)
assert.match(publicSite, /conecta personas, contratistas, documentos, requisitos, vencimientos y evidencia/)
assert.match(publicSite, /Preparar a organizaciones para la Ley 21\.719/)
assert.match(publicSite, /Data protection/)
assert.match(publicSite, /Mining/)
assert.match(publicSite, /Transport/)
assert.match(publicSite, /Construction/)
assert.match(publicSite, /Healthcare/)
assert.match(publicSite, /Agribusiness/)
