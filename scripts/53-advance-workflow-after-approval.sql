...
-- Fixes the golden-path deadlock where review moved the workflow to `running`
-- but left current_stage pointing at the already-approved stage.
...
begin
  ...
  if p_decision in ('approved', 'rejected', 'changes_requested')
     and coalesce(length(trim(p_comment)), 0) < 3 then
