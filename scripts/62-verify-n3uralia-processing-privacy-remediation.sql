...
begin
  ...
  where process.organization_id = v_organization_id
    ...
    and process.attributes ->> 'privacyNoticeEvidenceId' is not null
  ...
  if v_count <> 1 then
    raise exception 'Expected exactly one public privacy notice evidence for version 2026-08-03, found %.', v_count;
  end if;
  ...
  where link.organization_id = v_organization_id
    and link.evidence_id = v_notice_evidence_id
  ...
  where process.organization_id = v_organization_id
    ...
    and mission.metadata ->> 'privacyNoticeRequestId' = process.attributes ->> 'privacyNoticeRequestId'
    and mission.metadata ->> 'deletionEvidenceRequestId' = process.attributes ->> 'deletionEvidenceRequestId';
  ...
  where process.organization_id = v_organization_id
    ...
    and notice_request.project_id = (mission.metadata ->> 'projectId')::uuid
    and deletion_request.project_id = notice_request.project_id
    and notice_request.case_id is not distinct from (mission.metadata ->> 'caseId')::uuid
    and deletion_request.case_id is not distinct from notice_request.case_id
    and notice_request.control_id is not distinct from (mission.metadata ->> 'controlId')::uuid
    and deletion_request.control_id is not distinct from notice_request.control_id
    and notice_request.requested_from = process.owner_user_id
    and deletion_request.requested_from = process.owner_user_id
    and notice_request.title like 'Aviso aplicable y mapeado — %'
    and deletion_request.title like 'Evidencia de eliminación — %'
    ...
    and notice_request.due_at is not null
    and deletion_request.due_at is not null
    ...
    and (
      ...
      or deletion_request.submitted_evidence_id is not null
    );
  ...
  join public.organization_processes process
    on event.mission_id = (process.attributes ->> 'privacyRemediationMissionId')::uuid
  where process.organization_id = v_organization_id
    and process.process_type = 'processing_activity'
    and process.name in (
      'Gestión de contactos comerciales y solicitudes de demostración',
    ...
    and event.organization_id = v_organization_id
    and event.event_type = 'mission_created';

  if v_count <> 3 then
    raise exception 'Expected three mission creation events, found %.', v_count;
  end if;
...
select jsonb_build_object(
  ...
  'missions', (
    ...
    where mission.organization_id = (select id from n3uralia)
      and mission.metadata ->> 'source' = 'processing_privacy_and_deletion_remediation'
  ),
  'noticeRequests', (
    select count(*)
  ...
  'caseEvents', (
    select count(*)
    from public.compliance_case_events event
    where event.organization_id = (select id from n3uralia)
      and event.event_type = 'processing_privacy_remediation_ready'
  )
) as verification;
