-- KUMPLIO LeyChile production stabilization
-- Reproduces the fixes discovered during the first real Ley 21.719 capture.

begin;

-- Repeated identical captures are monitoring evidence and must not be unique.
...
-- Different legal sections can legitimately contain identical text.
...
drop function if exists private.invoke_leychile_bootstrap();
