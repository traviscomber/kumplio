...
5. continuidad operacional outcome-first integrada en la superficie canónica: Inicio prioriza una acción de cierre pendiente derivada de un outcome aprobado por sobre prioridades genéricas y conduce al usuario a `/app/casos/[id]/cierre`, conservando caso, workflow, evidencia y revisión humana sin obligarlo a descubrir una consola técnica separada.
6. la ficha canónica del caso muestra primero el resultado aprobado y el progreso de cierre verificable antes de exponer workspaces, assurance y trazabilidad secundaria; el usuario puede continuar exactamente la próxima acción de cierre desde el mismo contexto.
7. Outcome UX v4 incorpora, únicamente para `staging`, un transporte Cloudflare reproducible con Worker de ingreso autenticado, Queue y DLQ aisladas por nombre de ambiente. Esta capa solo prepara contexto seguro y nunca verifica acciones automáticamente; producción permanece fuera de alcance hasta evidencia end-to-end y una decisión posterior del owner.
...
**Decisión del owner — 20 de septiembre de 2026:** autorizar Worker + Queue + DLQ de Outcome UX v4 mediante Wrangler exclusivamente en entorno de prueba. Los recursos deben llevar sufijo `-staging`, apuntar solo a una Preview de Vercel, conservar revisión humana y permanecer separados de producción hasta validar el recorrido end-to-end.
