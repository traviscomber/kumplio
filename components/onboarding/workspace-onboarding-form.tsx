...
export function WorkspaceOnboardingForm({ initialEmail, initialOrganizationName, initialFirstName, initialLastName }: Props) {
  ...
  if (activationHandoff) {
    return (
      <section className="mx-auto max-w-3xl border-y border-primary/25 bg-primary/5 px-0 py-8 sm:py-10">
        <ShieldCheck className="h-6 w-6 text-primary" />
        <p className="mt-4 text-xs font-medium uppercase tracking-wider text-primary">Diagnóstico inicial preparado</p>
        <h2 className="font-heading mt-2 text-3xl font-normal tracking-[-0.02em]">Tu primer paso ya está claro</h2>
        <h3 className="font-heading mt-5 text-xl font-normal">{activationHandoff.title}</h3>
        <p className="mt-3 leading-7 text-muted-foreground">{activationHandoff.explanation}</p>
        <div className="mt-6 border-y border-border bg-background/60 px-0 py-4">
          <p className="text-xs font-medium uppercase tracking-wider text-primary">Siguiente acción recomendada</p>
          <p className="mt-1 font-medium">{activationHandoff.primaryLabel}</p>
        </div>
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          <Button asChild><Link href={activationHandoff.primaryHref}>Ir a mi siguiente acción</Link></Button>
          <Button asChild variant="outline"><Link href={activationHandoff.secondaryHref}>Ver mi inicio</Link></Button>
        </div>
  ...
  return <form onSubmit={handleSubmit} className="mx-auto max-w-3xl">
    ...
    {step === 0 && <Question title="¿Quién eres?" description="Kumplio adapta las preguntas y el trabajo visible, sin separarte en otro producto.">
      <div className="grid gap-3 md:grid-cols-3">{contexts.map(item => <Choice key={item.value} selected={userType === item.value} onClick={() => setUserType(item.value)}><strong className="font-medium">{item.label}</strong><span>{item.description}</span></Choice>)}</div>
    </Question>}
    {step === 1 && <Question title="¿Qué necesitas proteger, ordenar o resolver?" description="Cuéntalo con tus palabras. Esto define el primer caso, no una conclusión de cumplimiento.">
      <textarea autoFocus value={problem} onChange={e => setProblem(e.target.value)} rows={4} placeholder="Ej. Necesito ordenar contratos y vencimientos antes de una fiscalización" className="w-full rounded-[4px] border bg-background px-4 py-4 outline-none focus:ring-2 focus:ring-primary" />
      <div className="mt-4 flex flex-wrap gap-2">{intents.map(item => <button key={item} type="button" onClick={() => { setIntent(item); if (!problem) setProblem(item) }} className="rounded-[4px] border px-3 py-2 text-left text-xs font-medium hover:border-primary">{item}</button>)}</div>
    </Question>}
    ...
    {step === 3 && diagnosis && <Question title="Resultado inicial" description="Esto aún no acredita cumplimiento. Organiza el trabajo que debe confirmarse con evidencia.">
      <div className="border-y border-primary/25 bg-primary/5 px-0 py-5"><ShieldCheck className="h-5 w-5 text-primary" /><h3 className="font-heading mt-3 text-xl font-normal">{diagnosis.caseTitle}</h3><p className="mt-2 text-sm text-muted-foreground">{diagnosis.summary}</p><p className="mt-5 text-xs font-medium uppercase tracking-wider text-primary">Siguiente acción</p><p className="mt-1 font-medium">{diagnosis.nextAction.title}</p></div>
    ...
    {error && <p role="alert" className="mt-6 rounded-[4px] border border-destructive/30 bg-destructive/10 p-4 text-sm text-destructive">{error}</p>}
...
function Progress({ step }: { step: number }) { return <div className="mb-10 flex items-center justify-between"><p className="text-sm font-medium text-primary">Paso {step + 1} de 4</p><div className="h-px w-40 overflow-hidden bg-muted"><div className="h-full bg-primary transition-all" style={{ width: `${(step + 1) * 25}%` }} /></div></div> }
function Question({ title, description, children }: { title: string; description: string; children: React.ReactNode }) { return <section><h2 className="font-heading text-3xl font-normal tracking-[-0.02em]">{title}</h2><p className="mb-7 mt-3 leading-7 text-muted-foreground">{description}</p>{children}</section> }
function Choice({ selected, onClick, children }: { selected: boolean; onClick: () => void; children: React.ReactNode }) { return <button type="button" aria-pressed={selected} onClick={onClick} className={`flex flex-col gap-2 rounded-[4px] border p-5 text-left ${selected ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/40'}`}>{children}</button> }
function Field({ label, children }: { label: string; children: React.ReactElement }) { return <label className="grid gap-2 text-sm font-medium"><span>{label}</span><span className="[&>input]:w-full [&>input]:rounded-[4px] [&>input]:border [&>input]:bg-background [&>input]:px-4 [&>input]:py-3">{children}</span></label> }
function SelectField<T extends string>({ label, value, onChange, options }: { label: string; value: T; onChange: (value: T) => void; options: T[] }) { return <label className="grid gap-2 text-sm font-medium"><span>{label}</span><select value={value} onChange={e => onChange(e.target.value as T)} className="rounded-[4px] border bg-background px-4 py-3">{options.map(x => <option key={x} value={x}>{x}</option>)}</select></label> }
function PreviewList({ title, items, empty }: { title: string; items: string[]; empty?: string }) { return <div className="rounded-[4px] border border-border p-5"><h3 className="font-heading font-normal">{title}</h3>{items.length ? <ul className="mt-3 space-y-2 text-sm text-muted-foreground">{items.map(x => <li key={x}>• {x}</li>)}</ul> : <p className="mt-3 text-sm text-muted-foreground">{empty}</p>}</div> }
