...
export function LeyChileCaptureButton({ enabled }: { enabled: boolean }) {
  ...
  async function capture() {
    ...
    try {
      ...
      const status = result.status === 'unchanged' ? 'sin cambios' : 'nueva versión registrada'
      setMessage(
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-muted-foreground">
              Consulta el JSON oficial de BCN, conserva el original, calcula hashes, normaliza artículos e incisos y registra comprobaciones repetidas sin duplicar versiones.
            </p>
          </div>
        </div>
