...
function decodeEntities(value: string) {
  const named: Record<string, string> = {
    amp: "&", lt: "<", gt: ">", quot: '"', apos: "'", nbsp: " ", ndash: "–", mdash: "—", deg: "°",
  };
  return value
...
type SourcePart = { id: number; name: string; html: string };
...
function flattenHtml(entries: unknown, names: Map<number, string>) {
  ...
  const visit = (items: unknown) => {
    ...
    for (const item of items) {
      ...
      parts.push({ id, name: names.get(id) || `Parte ${id}`, html: String(object.t || "") });
      if (Array.isArray(object.h)) visit(object.h);
    }
  };
  visit(entries);
...
async function parsePayload(payload: Record<string, unknown>) {
  ...
  for (const part of parts) {
    ...
    const articleStarts: Array<{ index: number; label: string; reference: string; opening: string }> = [];
    paragraphs.forEach((paragraph, index) => {
      const match = paragraph.match(ARTICLE_PATTERN);
      if (match) articleStarts.push({
        index,
  ...
  return { parts, sections, normalizedDocument, documentHash: await sha256(normalizedDocument) };
}
...
Deno.serve(async () => {
  try {
    ...
    if (!supabaseUrl || !serviceKey) return json({ error: "missing_supabase_configuration" }, 500);
    ...
    const { data: source, error: sourceError } = await supabase
      ...
      .single();
    if (sourceError || !source) return json({ error: "source_not_registered" }, 500);
    ...
    if (!response.ok) return json({ error: "http_error", status: response.status }, 502);

    const rawJson = await response.text();
    const byteSize = new TextEncoder().encode(rawJson).byteLength;
    if (byteSize > MAX_BYTES) return json({ error: "response_too_large", byteSize }, 413);
    ...
    const { data, error } = await supabase.rpc("record_leychile_capture_bundle", {
      ...
      p_document_title: String(metadata.titulo_norma || "Ley 21.719 — Protección y tratamiento de datos personales"),
      p_document_type: "law",
    ...
    if (error) return json({ error: "capture_bundle_failed", code: error.code, detail: error.message }, 500);
