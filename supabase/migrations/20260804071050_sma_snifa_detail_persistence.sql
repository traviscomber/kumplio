...
begin
  ...
  expected_url := format(
    'https://snifa.sma.gob.cl/Sancionatorio/Ficha/%s',
    p_sma_process_id
  );
  ...
  select exists(
    ...
    where source_fetch.id = p_source_fetch_id
      and source_fetch.source_id = source_id
      and source_fetch.requested_url = expected_url
      and source_fetch.final_url = expected_url
      and source_fetch.content_hash = p_content_hash
      and source_fetch.status in ('succeeded', 'unchanged')
  ) into source_fetch_matches;

  if not source_fetch_matches then
  ...
  from jsonb_to_recordset(p_associations) as item(association_type text)
  where item.association_type = 'inspection';

  select count(*) into provisional_measure_count
  from jsonb_to_recordset(p_associations) as item(association_type text)
  where item.association_type = 'provisional_measure';
  ...
  insert into public.sma_sanctioning_detail_versions(
    sma_process_id, source_fetch_id, content_hash, payload_hash,
    parser_version, expediente, start_date, end_date, process_state,
    unit_count, holder_count, document_count, fact_count,
    inspection_count, provisional_measure_count, sanction_count,
    validation_status, metadata
  ) values (
    p_sma_process_id, p_source_fetch_id, p_content_hash, p_payload_hash,
    p_parser_version, p_expediente, p_start_date, p_end_date,
    p_process_state, unit_count, holder_count, document_count, fact_count,
    inspection_count, provisional_measure_count, sanction_count, 'pending',
  ...
  perform private.insert_sma_detail_units(created_version.id, p_units);
  perform private.insert_sma_detail_holders(created_version.id, p_holders);
  perform private.insert_sma_detail_documents(created_version.id, p_documents);
  perform private.insert_sma_detail_facts(created_version.id, p_facts);
  perform private.insert_sma_detail_associations(created_version.id, p_associations);
  perform private.insert_sma_detail_sanctions(created_version.id, p_sanctions);

  insert into public.sma_sanctioning_detail_heads(
    sma_process_id, current_version_id, updated_at
  ) values (
    p_sma_process_id, created_version.id, now()
  )
