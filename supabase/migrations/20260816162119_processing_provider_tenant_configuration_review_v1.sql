...
begin
  ...
  where evidence.id = p_evidence_id and evidence.organization_id = p_organization_id and evidence.project_id = v_request.project_id
  for update;
  ...
  if exists (select 1 from jsonb_array_elements(v_source_refs) source_ref
    where nullif(btrim(coalesce(source_ref ->> 'type','')), '') is null
       ...
       or nullif(btrim(coalesce(source_ref ->> 'capturedAt','')), '') is null) then
  ...
  if v_kind = 'supabase_backup_pitr' then
    ...
    if v_daily_backups_observed is not true or v_pitr_state not in ('enabled','disabled') or v_recovery_window_days is null or v_recovery_window_days < 0 or v_recovery_window_days > 90 then
    ...
    if v_openai_mode not in ('standard','modified_abuse_monitoring','zero_data_retention') or v_project_binding_observed is not true then
  ...
  set attributes=coalesce(process.attributes,'{}'::jsonb) || jsonb_build_object('providerTenantConfigurationStatus','verified','providerTenantConfigurationEvidenceId',p_evidence_id,'providerTenantConfigurationSnapshotHash',v_snapshot_hash,'providerTenantConfigurationKind',v_kind,'providerTenantConfigurationObservedAt',v_configuration_as_of,'providerTenantConfigurationReviewedAt',now()), updated_at=now()
  where process.id=p_process_id and process.organization_id=p_organization_id;

  update public.organization_vendors vendor
  set attributes=coalesce(vendor.attributes,'{}'::jsonb) || jsonb_build_object('tenantConfigurationStatus','verified','tenantConfigurationEvidenceId',p_evidence_id,'tenantConfigurationSnapshotHash',v_snapshot_hash,'tenantConfigurationKind',v_kind,'tenantConfigurationObservedAt',v_configuration_as_of,'tenantConfigurationReviewedAt',now()), updated_at=now()
  ...
  if v_request.case_id is not null then
    ...
    values(p_organization_id,v_request.case_id,p_actor_id,'processing_provider_tenant_configuration_verified','Configuración tenant del proveedor verificada con evidencia aceptada',jsonb_build_object('process_id',p_process_id,'vendor_id',v_vendor.id,'evidence_id',p_evidence_id,'configuration_kind',v_kind,'configuration_as_of',v_configuration_as_of,'snapshot_hash',v_snapshot_hash));
  end if;
...
comment on function public.promote_processing_provider_tenant_configuration_v1(uuid,uuid,uuid,uuid,uuid) is 'Promotes provider tenant configuration to verified only after an accepted evidence request with integrity-verified, provider-specific effective configuration evidence. Does not prove final deletion or provider backup purge.';
