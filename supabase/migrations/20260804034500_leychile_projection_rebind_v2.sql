...
update public.regulatory_document_sections s
set section_hash = encode(
      extensions.digest(
        pg_catalog.convert_to(s.section_key || E'\n' || s.normalized_text, 'UTF8'),
        'sha256'
      ),
      'hex'
    ),
...
where s.version_id in (
  ...
  where v.parser_version = 'leychile-official-json-v2'
);
