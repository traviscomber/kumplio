...
--
-- A lifecycle review does not overwrite the initial inventory review. It records
-- separate conclusions for legal basis, retention, recipients, subprocessors
-- and international transfers, with evidence, unknowns and human approval.
...
create table if not exists public.processing_activity_lifecycle_reviews (
  ...
  constraint processing_activity_lifecycle_request_unique
    unique (organization_id, request_key),
  constraint processing_activity_lifecycle_version_unique
    unique (process_id, version),
  constraint processing_activity_lifecycle_version_check
    check (version > 0),
  constraint processing_activity_lifecycle_decision_check
    check (decision in ('approved', 'changes_requested', 'rejected')),
  constraint processing_activity_lifecycle_basis_status_check
    check (basis_status in ('validated', 'needs_changes', 'pending_evidence', 'not_applicable')),
  constraint processing_activity_lifecycle_retention_status_check
    check (retention_status in ('validated', 'needs_changes', 'pending_evidence', 'not_applicable')),
  constraint processing_activity_lifecycle_recipients_status_check
    check (recipients_status in ('validated', 'needs_changes', 'pending_evidence', 'not_applicable')),
  constraint processing_activity_lifecycle_subprocessors_status_check
    check (subprocessors_status in ('validated', 'needs_changes', 'pending_evidence', 'not_applicable')),
  constraint processing_activity_lifecycle_transfers_status_check
    check (transfers_status in ('validated', 'needs_changes', 'pending_evidence', 'not_applicable')),
  constraint processing_activity_lifecycle_recipients_json_check
    check (jsonb_typeof(recipients) = 'array'),
  constraint processing_activity_lifecycle_subprocessors_json_check
    check (jsonb_typeof(subprocessors) = 'array'),
  constraint processing_activity_lifecycle_transfers_json_check
    check (jsonb_typeof(transfers) = 'array'),
  constraint processing_activity_lifecycle_sources_json_check
    check (jsonb_typeof(source_refs) = 'array'),
  constraint processing_activity_lifecycle_hash_check
    check (snapshot_hash ~ '^[0-9a-f]{64}$'),
  constraint processing_activity_lifecycle_approved_check
    check (
      decision <> 'approved'
      or (
  ...
  constraint processing_activity_lifecycle_changes_check
    check (
...
create index if not exists processing_activity_lifecycle_org_process_idx
  on public.processing_activity_lifecycle_reviews (organization_id, process_id, version desc);
create index if not exists processing_activity_lifecycle_project_idx
  on public.processing_activity_lifecycle_reviews (project_id, reviewed_at desc);
create index if not exists processing_activity_lifecycle_evidence_idx
  on public.processing_activity_lifecycle_reviews (evidence_id);
create index if not exists processing_activity_lifecycle_supersedes_idx
  on public.processing_activity_lifecycle_reviews (supersedes_id);
...
create policy "processing lifecycle reviews browser deny"
  on public.processing_activity_lifecycle_reviews
  for all
...
begin
  ...
  if jsonb_typeof(v_recipients) <> 'array'
     or jsonb_typeof(v_subprocessors) <> 'array'
     or jsonb_typeof(v_transfers) <> 'array'
     or jsonb_typeof(v_source_refs) <> 'array' then
  ...
  if v_basis_status = 'validated'
     and (v_basis_type is null or v_basis_summary is null or char_length(v_basis_summary) < 10) then
    raise exception using errcode = '22023', message = 'Validated legal basis requires type and summary';
  end if;

  if v_retention_status = 'validated'
     and (v_retention_rule is null or v_retention_trigger is null or v_retention_period is null) then
  ...
  if v_decision = 'approved' and (
    not (v_basis_status = any(v_final_statuses))
    or not (v_retention_status = any(v_final_statuses))
    or not (v_recipients_status = any(v_final_statuses))
    or not (v_subprocessors_status = any(v_final_statuses))
    or not (v_transfers_status = any(v_final_statuses))
    or cardinality(v_unknowns) > 0
  ) then
    raise exception using errcode = '23514', message = 'Approved lifecycle review cannot retain unresolved dimensions or unknowns';
  end if;

  if v_decision = 'changes_requested'
     and cardinality(v_unknowns) = 0
     and v_basis_status = any(v_final_statuses)
     and v_retention_status = any(v_final_statuses)
     and v_recipients_status = any(v_final_statuses)
     and v_subprocessors_status = any(v_final_statuses)
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      p_organization_id::text || ':processing-lifecycle:' || p_process_id::text,
      21719
    )
  );
  ...
  if v_existing_review_id is null then
    select review.id, review.version
    into v_previous_review_id, v_version
    from public.processing_activity_lifecycle_reviews review
    where review.organization_id = p_organization_id
      and review.process_id = p_process_id
  ...
  v_snapshot := jsonb_build_object(
    ...
    'version', v_version,
    'supersedesId', v_previous_review_id,
    'decision', v_decision,
    'basis', jsonb_build_object(
      'status', v_basis_status,
      'type', v_basis_type,
      'summary', v_basis_summary
    ),
    'retention', jsonb_build_object(
      'status', v_retention_status,
      'rule', v_retention_rule,
      'trigger', v_retention_trigger,
      'period', v_retention_period
    ),
  ...
  set validation_status = 'accepted',
      ...
      metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object(
        'scope', 'processing_activity_lifecycle_review',
        'processingLifecycleRequestKey', p_request_key,
        'processId', p_process_id,
        'version', v_version,
        'decision', v_decision,
        'snapshotHash', v_snapshot_hash,
  ...
  insert into public.processing_activity_lifecycle_reviews (
    ...
    p_request_key,
    v_version,
    v_previous_review_id,
    v_decision,
    v_basis_status,
    v_retention_status,
    v_recipients_status,
    v_subprocessors_status,
    v_transfers_status,
    v_basis_type,
    v_basis_summary,
    v_retention_rule,
  ...
  set attributes = coalesce(process.attributes, '{}'::jsonb) || jsonb_build_object(
        'latestLifecycleReviewId', v_review_id,
        'lifecycleVersion', v_version,
        'lifecycleDecision', v_decision,
        'lifecycleStatuses', jsonb_build_object(
          'basis', v_basis_status,
          'retention', v_retention_status,
          'recipients', v_recipients_status,
          'subprocessors', v_subprocessors_status,
          'transfers', v_transfers_status
        ),
        'lifecycleUnknowns', to_jsonb(v_unknowns),
        'lifecycleReviewedAt', now(),
        'basisStatus', case
          when v_basis_status = 'validated' then 'validated'
          else coalesce(process.attributes ->> 'basisStatus', 'proposed')
        end
      ),
      updated_at = now()
  where process.id = p_process_id
  ...
  set attributes = coalesce(dataset.attributes, '{}'::jsonb) || jsonb_build_object(
        'basisStatus', v_basis_status,
        'retentionStatus', v_retention_status,
        'recipientsStatus', v_recipients_status,
        'subprocessorsStatus', v_subprocessors_status,
        'transfersStatus', v_transfers_status,
        'lifecycleReviewId', v_review_id,
        'lifecycleVersion', v_version
      ),
      retention_rule = case
        when v_retention_status = 'validated' and v_retention_rule is not null then v_retention_rule
        else dataset.retention_rule
      end,
      updated_at = now()
  from public.organization_process_datasets process_dataset
  where process_dataset.process_id = p_process_id
    and dataset.id = process_dataset.dataset_id
  ...
  join public.organization_process_assets process_asset
    on process_asset.asset_id = vendor_asset.asset_id
  where process_asset.process_id = p_process_id
    and vendor.id = vendor_asset.vendor_id
  ...
  if v_case_id is not null then
    insert into public.compliance_case_events (
      ...
      'Base y ciclo de vida del tratamiento revisados',
      jsonb_build_object(
        ...
        'version', v_version,
        'decision', v_decision,
        'basis_status', v_basis_status,
        'retention_status', v_retention_status,
        'recipients_status', v_recipients_status,
        'subprocessors_status', v_subprocessors_status,
        'transfers_status', v_transfers_status,
...
revoke all on function public.review_processing_activity_lifecycle_v1(uuid, uuid, uuid, uuid, jsonb)
  from public, anon, authenticated;
grant execute on function public.review_processing_activity_lifecycle_v1(uuid, uuid, uuid, uuid, jsonb)
