...
export default async function ActivityPage() {
  ...
  const items = buildOperationalActivity({
    caseEvents: (events || []).map((event) => ({
      ...
      caseTitle: caseTitleById.get(String(event.case_id)) || null,
      eventType: String(event.event_type || ''),
      summary: typeof event.summary === 'string' ? event.summary : null,
      createdAt: String(event.created_at),
    })),
    continuousReviews: timeline.map((item) => ({
      id: item.id,
      date: item.date,
      headline: item.headline,
      changesFound: item.changesFound,
      criticalItems: item.criticalItems,
    })),
    limit: 50,
  })
    ...
    <main className="container mx-auto max-w-5xl px-4 py-8 sm:px-6">
      <header className="mb-8">
        <div className="flex items-center gap-3 text-primary">
          ...
          <p className="text-sm font-semibold">Trazabilidad operativa</p>
        </div>
        <h1 className="mt-3 text-3xl font-black tracking-tight sm:text-4xl">Actividad reciente</h1>
        <p className="mt-3 max-w-2xl leading-7 text-muted-foreground">
          Un historial legible de acciones y revisiones ya registradas en tu organización.
        </p>
      </header>
      ...
      {items.length === 0 ? (
        <section className="rounded-2xl border border-dashed bg-card p-8 text-center">
          ...
          <h2 className="mt-4 text-xl font-bold">Todavía no hay actividad reciente para mostrar.</h2>
          <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-muted-foreground">
            Cuando existan acciones, revisiones o cambios persistidos, aparecerán aquí en orden cronológico.
          </p>
        </section>
      ) : (
        <ol className="divide-y border-y border-border/70" aria-label="Actividad reciente">
...
function ActivityRow({ item }: { item: OperationalActivityItem }) {
  return (
    <li className="grid gap-4 py-5 sm:grid-cols-[150px_minmax(0,1fr)_auto] sm:items-start">
      <time className="text-xs font-semibold text-muted-foreground" dateTime={item.occurredAt}>
        {formatDate(item.occurredAt)}
      </time>
      <div className="min-w-0">
        <p className="font-bold">{item.label}</p>
        {item.detail && <p className="mt-1 break-words text-sm leading-6 text-muted-foreground">{item.detail}</p>}
        {item.context && <p className="mt-2 break-words text-xs font-semibold text-muted-foreground">{item.context}</p>}
      </div>
      <Link
        href={item.href}
        className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-bold transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
      >
