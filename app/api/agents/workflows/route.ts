...
export async function POST(req: NextRequest) {
  ...
  if (!complianceCase) {
    return NextResponse.json({ error: 'Compliance case not found' }, { status: 404 })
  }
  ...
  const { data: activeWorkflow } = await supabase
    ...
    .in('status', ['draft', 'running', 'paused', 'pending_review'])
    .limit(1)
    .maybeSingle()

  if (activeWorkflow) {
    return NextResponse.json(
      { error: 'An active workflow of this type already exists for the case', code: 'active_workflow_exists', workflowId: activeWorkflow.id },
      { status: 409 },
    )
  }
  ...
  if (linksError) {
    return NextResponse.json({ error: 'Unable to read case resources', code: 'case_context_unavailable' }, { status: 500 })
  }
  ...
  try {
    ...
    if (error || !workflowId) {
      if (error?.code === '23505') {
        return NextResponse.json({ error: 'An active workflow already exists', code: 'active_workflow_exists' }, { status: 409 })
      }
