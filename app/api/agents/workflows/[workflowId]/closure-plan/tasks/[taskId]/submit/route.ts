...
export async function POST(request: NextRequest, context: { params: Promise<{ workflowId: string; taskId: string }> }) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: 'Select at least one verified evidence item', code: 'invalid_closure_evidence', details: parsed.error.flatten() }, { status: 400 })
  }
  ...
  const { data, error } = await admin.rpc('submit_outcome_closure_task', {
    p_actor_id: user.id,
    p_organization_id: organizationId,
    p_task_id: taskId,
    p_evidence_ids: [...new Set(parsed.data.evidenceIds)],
    p_note: parsed.data.note || null,
  })

  if (error) {
    const message = error.message || ''
    const code = message.includes('closure_evidence_not_verified') ? 'closure_evidence_not_verified'
      : message.includes('closure_evidence_required') ? 'closure_evidence_required'
        : message.includes('closure_task_not_found') ? 'closure_task_not_found'
          : 'closure_task_submit_failed'
    const status = code === 'closure_task_not_found' ? 404 : code === 'closure_task_submit_failed' ? 500 : 409
    if (status >= 500) console.error('[agents/closure-plan/task-submit]', error.code)
    return NextResponse.json({ error: 'Unable to prepare this action for closure review', code }, { status })
  }
