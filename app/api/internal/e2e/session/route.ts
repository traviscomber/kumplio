...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.success) {
      return NextResponse.json({ error: 'Invalid E2E session request', code: 'invalid_request' }, { status: 400 })
    }
    ...
    if (caseError || !complianceCase) {
      return NextResponse.json({ error: 'Golden-path case not found', code: 'case_not_found' }, { status: 404 })
    }
    ...
    if (!guidedKey.startsWith('golden-')) {
      return NextResponse.json({ error: 'Case is not authorized for programmatic E2E', code: 'case_not_authorized' }, { status: 403 })
    }
    ...
    const sessionClient = createServerClient(url, key, {
      cookies: {
        getAll() {
          return cookiesToWrite.map(({ name, value }) => ({ name, value }))
        },
        setAll(values) {
          cookiesToWrite = values
        },
      },
    })

    const { data: sessionData, error: sessionError } = await sessionClient.auth.signInWithPassword({
      email: SERVICE_EMAIL,
      password,
    })
