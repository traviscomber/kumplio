...
async function collectGoldenPathState(admin: AdminClient, userId: string) {
  ...
  const assertions = {
    ...
    oneCompletedWorkflow: workflowRows.length === 1 && workflow?.status === 'completed' && Number(workflow?.total_stages) === 3,
    threeApprovedStages: stages.length === 3 && stages.every((stage) => stage.status === 'approved' && stage.run_id),
    threeApprovedRuns: runs.length === 3 && runs.every((run) => run.status === 'approved'),
    threeApprovedArtifacts: activeArtifacts.length === 3 && activeArtifacts.every((artifact) => artifact.status === 'approved'),
    threeApprovedHumanReviews: reviews.length === 3 && reviews.every((review) => review.decision === 'approved'),
    durableQueueCompleted: jobs.length === 3 && activeJobs.length === 0 && deadLetters.length === 0 && jobs.every((job) => job.status === 'succeeded'),
