...
export async function POST(
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { error: 'Invalid evidence submission', code: 'invalid_request', details: parsed.error.flatten() },
      { status: 400 },
    )
  }
  ...
  if (!['open', 'changes_requested'].includes(evidenceRequest.status)) {
    return NextResponse.json({ error: 'Evidence request is not open for submission', code: 'invalid_request_state' }, { status: 409 })
  }
  ...
  const { data: evidence } = await supabase
    ...
    .eq('organization_id', organizationId)
    .eq('project_id', evidenceRequest.project_id)
    .maybeSingle()

  if (!evidence) {
    return NextResponse.json({ error: 'Evidence not found in this project', code: 'evidence_not_found' }, { status: 404 })
  }

  try {
    const admin = createAdminClient()
    const { error } = await admin.rpc('submit_evidence_request_record', {
      p_actor_id: user.id,
    ...
    if (error) {
      console.error('[evidence/requests/submit]', error.code)
      return NextResponse.json({ error: 'Unable to submit evidence', code: 'evidence_submit_failed' }, { status: 500 })
    }

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('[evidence/requests/submit/configuration]', error instanceof Error ? error.message : 'unknown')
