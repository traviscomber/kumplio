    ...
    <div className="min-h-screen overflow-x-hidden bg-[#151513] text-[#C2A887]">
      ...
      <main>
        ...
        <section id="como-funciona" className="relative min-h-[720px] scroll-mt-20 border-b border-white/10 md:min-h-[800px]">
          ...
          <div className="relative mx-auto max-w-[1440px] px-5 py-20 sm:px-8 md:py-28 lg:px-12"><p className={eyebrowClass}>{copy.journey.eyebrow}</p><h2 className={h2Class}>{copy.journey.title}</h2><div className="mt-56 grid border-y border-white/10 bg-[#151513]/80 backdrop-blur-md md:mt-[300px] md:grid-cols-4">{copy.journey.steps.map((step,index)=><article key={step.title} className="border-b border-white/10 p-7 md:border-b-0 md:border-r last:md:border-r-0"><span className="text-[11px] text-[#A7C63A]">0{index+1}</span><h3 className="mt-6 text-2xl font-light text-[#D5B994]">{step.title}</h3><p className="mt-4 text-sm leading-7 text-[#AAA08F]">{step.description}</p></article>)}</div></div>
        </section>

        <section id="verticales" className="relative min-h-[900px] scroll-mt-20 border-b border-white/10"><CinematicImage src="/brand/kumplio-mining-transport.webp" opacityClass="opacity-80" /><div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(21,21,19,.48)_0%,rgba(21,21,19,.04)_28%,rgba(21,21,19,.88)_100%)]"/><div className="relative mx-auto max-w-[1440px] px-5 py-20 sm:px-8 md:py-28 lg:px-12"><p className={eyebrowClass}>{copy.scenarios.eyebrow}</p><h2 className={h2Class}>{copy.scenarios.title}</h2><div className="mt-52 grid border-l border-t border-white/10 bg-black/40 backdrop-blur-sm sm:grid-cols-2 md:mt-[260px] lg:grid-cols-3">{copy.scenarios.items.map((item,index)=>{const slug=item.slug as VerticalSlug;return <Link key={item.slug} href={getPublicSiteHref(`/verticales/${item.slug}`, locale)} className="group relative flex min-h-[320px] flex-col overflow-hidden border-b border-r border-white/10 p-7 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#A7C63A]"><Image src={VERTICAL_IMAGES[slug]} alt="" fill sizes="(min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw" style={{objectPosition:VERTICAL_IMAGE_POSITIONS[slug]}} className="object-cover opacity-60 transition duration-500 group-hover:scale-[1.03] group-hover:opacity-70" /><span className="absolute inset-0 bg-[linear-gradient(180deg,rgba(10,10,9,.06)_0%,rgba(10,10,9,.28)_42%,rgba(10,10,9,.88)_100%)] transition-colors group-hover:bg-[linear-gradient(180deg,rgba(10,10,9,.02)_0%,rgba(10,10,9,.22)_42%,rgba(10,10,9,.84)_100%)]" /><div className="relative flex h-full flex-1 flex-col"><p className="text-[11px] text-[#B5D83D]">{String(index+1).padStart(2,'0')}</p><div className="mt-auto pt-24"><h3 className="text-2xl font-light text-[#F0D7B6]">{item.title}</h3><p className="mt-4 text-sm leading-7 text-[#D2C7B7]">{item.description}</p><span className="mt-6 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[.14em] text-[#B5D83D]">{item.action}<ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span></div></div></Link>})}</div></div></section>

        <section id="producto" className="relative min-h-[760px] scroll-mt-20 border-b border-white/10"><CinematicImage src="/brand/kumplio-operating-model.webp" /><div className="absolute inset-0 bg-[linear-gradient(180deg,#151513_0%,transparent_35%,#151513_100%)]"/><div className="relative mx-auto max-w-[1440px] px-5 py-20 sm:px-8 md:py-28 lg:px-12"><p className={eyebrowClass}>{copy.workflow.eyebrow}</p><h2 className={h2Class}>{copy.workflow.title}</h2><p className="mt-6 max-w-2xl text-base leading-8 text-[#AAA08F]">{copy.workflow.description}</p><div className="mt-56 grid border-y border-white/10 bg-[#151513]/80 backdrop-blur-md md:mt-[290px] md:grid-cols-3">{copy.workflow.stages.map((stage,index)=>{const Icon=workflowIcons[index];return <article key={stage.title} className="p-7 md:border-r last:md:border-r-0"><Icon className="h-6 w-6 text-[#B17A4D]"/><h3 className="mt-7 text-3xl font-light text-[#D5B994]">{stage.title}</h3><p className="mt-4 text-sm leading-7 text-[#AAA08F]">{stage.description}</p></article>})}</div></div></section>

        <section className="relative min-h-[820px] border-b border-white/10"><CinematicImage src="/brand/kumplio-specialists.webp"/><div className="absolute inset-0 bg-[linear-gradient(180deg,#151513_0%,transparent_35%,#151513_100%)]"/><div className="relative mx-auto max-w-[1440px] px-5 py-20 sm:px-8 md:py-28 lg:px-12"><p className={eyebrowClass}>{copy.specialists.eyebrow}</p><h2 className={h2Class}>{copy.specialists.title}</h2><p className="mt-6 max-w-2xl text-base leading-8 text-[#AAA08F]">{copy.specialists.description}</p><div className="mt-60 grid border-y border-white/10 bg-[#151513]/80 backdrop-blur-md md:mt-[310px] md:grid-cols-3">{copy.specialists.people.map(person=><article key={person.name} className="border-b border-white/10 p-7 md:border-b-0 md:border-r last:md:border-r-0"><p className="text-[10px] uppercase tracking-[.16em] text-[#A7C63A]">{person.label}</p><h3 className="mt-4 text-2xl font-light text-[#D5B994]">{person.name}</h3><p className="mt-3 text-sm leading-7 text-[#AAA08F]">{person.description}</p></article>)}</div><div className="mt-6 flex flex-wrap gap-x-6 gap-y-3">{copy.specialists.capabilities.map(capability=><span key={capability} className="text-xs uppercase tracking-[.12em] text-[#AAA08F]">{capability}</span>)}</div><p className="mt-7 max-w-3xl text-sm leading-7 text-[#AAA08F]">{copy.specialists.note}</p></div></section>
