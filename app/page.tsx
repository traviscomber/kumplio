...
import { ArrowRight, CheckCircle2, Eye, FileCheck2, SearchCheck, ShieldCheck } from 'lucide-react'
...
import { VERTICAL_IMAGES, type VerticalSlug } from '@/lib/i18n/vertical-public-copy'
    ...
    <div className="min-h-screen overflow-x-hidden bg-[#151513] text-[#C2A887]">
      <nav aria-label="Primary" className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[#151513]/90 backdrop-blur-xl">
        <div className="mx-auto flex h-20 max-w-[1440px] items-center justify-between px-5 sm:px-8 lg:px-12">
          <Link href={withPublicLocale('/', locale)} aria-label="Kumplio" className="shrink-0"><Image src="/kumplio-logo-canonical.png" alt="Kumplio" width={455} height={171} priority className="h-auto w-[132px] object-contain sm:w-[150px]" /></Link>
      ...
      <main>
        ...
        <section id="verticales" className="relative min-h-[980px] scroll-mt-20 border-b border-white/10"><CinematicImage src="/brand/kumplio-mining-transport.webp" /><div className="absolute inset-0 bg-[linear-gradient(180deg,#151513_0%,transparent_25%,#151513_100%)]"/><div className="relative mx-auto max-w-[1440px] px-5 py-24 sm:px-8 md:py-32 lg:px-12"><p className={eyebrowClass}>{copy.scenarios.eyebrow}</p><h2 className={h2Class}>{copy.scenarios.title}</h2><div className="mt-[300px] grid border-l border-t border-white/10 bg-black/55 backdrop-blur-md sm:grid-cols-2 lg:grid-cols-3">{copy.scenarios.items.map((item,index)=><Link key={item.slug} href={getPublicSiteHref(`/verticales/${item.slug}`, locale)} className="group relative flex min-h-[330px] flex-col overflow-hidden border-b border-r border-white/10 p-7 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#A7C63A]"><Image src={VERTICAL_IMAGES[item.slug as VerticalSlug]} alt="" fill sizes="(min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw" className="object-cover opacity-40 transition duration-500 group-hover:scale-[1.03] group-hover:opacity-55" /><span className="absolute inset-0 bg-[linear-gradient(180deg,rgba(10,10,9,.18)_0%,rgba(10,10,9,.94)_72%)] transition-colors group-hover:bg-[linear-gradient(180deg,rgba(10,10,9,.1)_0%,rgba(10,10,9,.9)_72%)]" /><div className="relative flex h-full flex-1 flex-col"><p className="text-[11px] text-[#A7C63A]">{String(index+1).padStart(2,'0')}</p><div className="mt-auto pt-24"><h3 className="text-2xl font-light text-[#E0C5A1]">{item.title}</h3><p className="mt-4 text-sm leading-7 text-[#C3B7A5]">{item.description}</p><span className="mt-6 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[.14em] text-[#A7C63A]">{item.action}<ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span></div></div></Link>)}</div></div></section>
