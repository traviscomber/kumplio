  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#151513] text-[#C2A887]">
      ...
      <main>
        ...
        <section id="verticales" className="relative min-h-[980px] scroll-mt-20 border-b border-white/10"><CinematicImage src="/brand/kumplio-mining-transport.webp" /><div className="absolute inset-0 bg-[linear-gradient(180deg,#151513_0%,transparent_25%,#151513_100%)]"/><div className="relative mx-auto max-w-[1440px] px-5 py-24 sm:px-8 md:py-32 lg:px-12"><p className={eyebrowClass}>{copy.scenarios.eyebrow}</p><h2 className={h2Class}>{copy.scenarios.title}</h2><div className="mt-[300px] grid border-l border-t border-white/10 bg-black/40 backdrop-blur-sm sm:grid-cols-2 lg:grid-cols-3">{copy.scenarios.items.map((item,index)=>{const slug=item.slug as VerticalSlug;return <Link key={item.slug} href={getPublicSiteHref(`/verticales/${item.slug}`, locale)} className="group relative flex min-h-[350px] flex-col overflow-hidden border-b border-r border-white/10 p-7 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#A7C63A]"><Image src={VERTICAL_IMAGES[slug]} alt="" fill sizes="(min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw" style={{objectPosition:VERTICAL_IMAGE_POSITIONS[slug]}} className="object-cover opacity-60 transition duration-500 group-hover:scale-[1.03] group-hover:opacity-70" /><span className="absolute inset-0 bg-[linear-gradient(180deg,rgba(10,10,9,.06)_0%,rgba(10,10,9,.28)_42%,rgba(10,10,9,.88)_100%)] transition-colors group-hover:bg-[linear-gradient(180deg,rgba(10,10,9,.02)_0%,rgba(10,10,9,.22)_42%,rgba(10,10,9,.84)_100%)]" /><div className="relative flex h-full flex-1 flex-col"><p className="text-[11px] text-[#B5D83D]">{String(index+1).padStart(2,'0')}</p><div className="mt-auto pt-28"><h3 className="text-2xl font-light text-[#F0D7B6]">{item.title}</h3><p className="mt-4 text-sm leading-7 text-[#D2C7B7]">{item.description}</p><span className="mt-6 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[.14em] text-[#B5D83D]">{item.action}<ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span></div></div></Link>})}</div></div></section>
