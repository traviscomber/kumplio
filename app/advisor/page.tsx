        ...
        <section className="overflow-hidden rounded-[28px] border bg-card shadow-sm">
          <div className="border-b bg-[radial-gradient(circle_at_top_left,hsl(var(--primary)/0.12),transparent_42%)] px-6 py-10 sm:px-10 sm:py-14">
            ...
            <div className="mt-10 space-y-4">
              <BriefLine icon={<CheckCircle2 className="h-5 w-5" />} label="Información revisada" value={`${reviewedItems} elementos considerados`} />
              <BriefLine icon={<ShieldCheck className="h-5 w-5" />} label="Situaciones abiertas" value={`${summary.openSituations}`} />
              <BriefLine icon={<CheckCircle2 className="h-5 w-5" />} label="Decisiones preparadas" value={`${summary.pendingDecisions}`} />
              <BriefLine icon={<Clock3 className="h-5 w-5" />} label="Tiempo estimado" value={`${summary.estimatedMinutes} minutos`} />
            </div>
            ...
            <div className="mt-10 flex flex-col gap-3 sm:flex-row">
              ...
              <Link href="/cases/new" className="inline-flex min-h-12 items-center justify-center rounded-xl border bg-background/70 px-6 py-3 font-bold transition hover:border-primary/40 hover:bg-muted/50">
                <Plus className="mr-2 h-4 w-4" /> Preparar un caso
              </Link>
            </div>
          </div>
          ...
          <div className="grid gap-8 px-6 py-8 sm:px-10 lg:grid-cols-[1fr_0.8fr]">
                  ...
                  <Link key={`${item.href}-${item.id}`} href={item.href} className="block rounded-2xl border bg-background/50 p-5 transition hover:border-primary/30 hover:bg-muted/40">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="mt-3 text-xs text-muted-foreground">Riesgo {riskLabel(item.reasoning.risk)} · confianza {item.reasoning.confidence}%</p>
                      </div>
            ...
            <aside className="rounded-2xl border border-primary/20 bg-primary/5 p-6">
              ...
              <p className="mt-3 text-sm leading-6 text-muted-foreground">Kumplio ya ordenó las situaciones y decisiones disponibles para que no tengas que revisar cada documento o alerta por separado.</p>
...
function BriefLine({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div className="flex items-center justify-between gap-4 border-b pb-4 last:border-0 last:pb-0"><div className="flex items-center gap-3 text-primary">{icon}<span className="text-sm font-semibold text-foreground">{label}</span></div><span className="text-sm font-bold text-foreground">{value}</span></div>
}
