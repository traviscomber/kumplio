...
import { History, Send, Archive, CheckCircle2 } from 'lucide-react'
...
function Actions({snapshot}:{snapshot:Snapshot}) { const actions = snapshot.status==='draft'?[['submit','Enviar a revisión',Send]]:snapshot.status==='review_required'?[['approve','Aprobar',CheckCircle2],['reject','Devolver',Archive]]:snapshot.status==='approved'?[['publish','Publicar',Send]]:snapshot.status==='published'?[['archive','Archivar',Archive]]:[]; return <div className="flex flex-wrap gap-2">{actions.map(([action,label,Icon])=><form key={String(action)} action={transitionSnapshotAction}><input type="hidden" name="id" value={snapshot.id}/><input type="hidden" name="action" value={String(action)}/><button className="inline-flex items-center gap-2 rounded-xl border px-4 py-2 text-sm font-bold"><Icon className="h-4 w-4"/>{String(label)}</button></form>)}</div> }
